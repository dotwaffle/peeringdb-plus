...
- On `poc`, `ixlan`, `ixpfx`, `netixlan`, `netfac`, `ixfac` and `carrierfac`, any other value returns no rows, as upstream.
  ...
  So does a `since`, `limit` or `skip` that is not an integer, and a `depth` on a single-object GET that is not an integer.
...
| A query key that upstream handles in Python and that is not a model field: `hide_ix_no_fac` on `ix`, `ixlan`, `net` and `netixlan` | Filters in Python before the model-field filters: the `hide_ix_no_fac` mixin (2.83.0 `rest.py:1267-1297`). | Silently ignored, like any unknown key: HTTP 200 with the unfiltered list. A single-object GET ignores it too and returns the object; upstream returns `404` when the key excludes the object, for example `/api/ix/<id>?hide_ix_no_fac=1` for an exchange with no facility (`rest.py:752-753`). | The relation keys, the presence keys (`asn_overlap` included), `ix?ipblock=`, `ixpfx?whereis=`, `ix?capacity=` and the `fac` and `org` `distance` key of a `prepare_query`, and `name_search`, are not in this row: they filter as upstream does or have their own rows (see § Relation filters, § Presence filters, § IP block filter, § IP address lookup, § Capacity filter, § Distance filter and § Name search). The model-field filters that pdbcompat mirrors do not cover `hide_ix_no_fac`. Implement the key when a client needs it. Locked by `TestParity_Traversal/DIVERGENCE_prepare_query_keys_silent_ignore`. | v1.1 (registered 2026-09-23) |
