...
The message is the upstream text for a lookup by `id` or `asn` with no match (`Entity not found`) and for a method that upstream does not map to a handler (`Method "PUT" not allowed.`).
