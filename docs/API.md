...
| `?depth=N` on list endpoints (any `/api/<type>` without a pk) | Upstream `rest.py:744-748` accepts `?depth=` on list requests and caps row count at `API_DEPTH_ROW_LIMIT=250`. | peeringdb-plus silently drops `?depth=` on list endpoints with a `slog.DebugContext` paper trail (`opts.Depth` is never threaded into list closures). `?depth=` on single-object GET (`/api/<type>/<id>`) expands the first level of nested objects as upstream specifies (see the depth-2 second-level-nesting row below for the one bounded exception). Functional list+depth is deferred indefinitely — the `budget.go` memory envelope would refuse the resulting response sizes on 256 MB replicas anyway. | Memory envelope on 256 MB replicas — the 13-entity × 2-depth worst case exceeds the 128 MiB budget for any realistic row count. `docs/ARCHITECTURE.md § Response Memory Envelope` documents the per-entity ceiling. Parity-locked by `TestParity_Limit/depth_on_list_silently_dropped_DIVERGENCE`. | v1.16 |
...
| `GET /api/<type>/<id>?depth=2` second-level nested FK objects (e.g. the `ix` embedded under an ixlan, or `net`/`fac` embedded under a leaf join row such as netfac/netixlan) | Upstream expands every FK reference within the depth budget, so a second-level nested object still carries its own reverse relations as bare ID lists, e.g. `ixlan.ix.ixlan_set = [<ids>]`. | peeringdb-plus expands the FIRST-level nested FK object faithfully — the `org` on net/ix/fac/carrier/campus carries net_set/fac_set/ix_set/carrier_set/campus_set as ID lists, and `fac.campus` carries fac_set plus a flat org — but renders SECOND-level nested FK objects as flat rows without their own ID-list sets. (Back-reference parent FKs are stripped from nested reverse-set elements, and the Facility serializer omits netfac/ixfac/carrierfac at depth=2, both matching upstream.) | The first level covers the overwhelming majority of real consumer access (`net.org`, `ix.org`, …); reproducing the full recursive depth budget for every leaf permutation is disproportionate effort for data almost no client reads two levels deep. Validated against live `www.peeringdb.com/api` payloads (2026-06-04). Locked by `internal/pdbcompat.TestDepth_UpstreamParityShape` (positive shape) and its `second_level_nested_fk_stays_flat_DIVERGENCE` sub-test. | v1.19.3 |
