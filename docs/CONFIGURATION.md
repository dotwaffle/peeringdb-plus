...
| `PDBPLUS_FULL_SYNC_INTERVAL` | No | `24h` | Go duration | Interval after which a sync cycle forces a full bare-list refetch of every type, regardless of the per-table `MAX(updated)` cursor. Defends against pathological upstream cross-row inconsistency in any `?since=`-based design (a response with row R' (`updated=M`) present but earlier row R (`updated < M`) missing → R is permanently missed without periodic full refetch). The forced cycle is recorded in `sync_status.mode='full'`; `GetLastSuccessfulFullSyncTime` reads that column to decide whether the next cycle must escalate. Single sync_status query per cycle (NOT per-type). Forced-full cycles over a populated table also fetch `?since=<cursor>` per type on top of the bare snapshot so the window's tombstones are captured (bare lists carry only live rows: `status='ok'`, plus `not-operational` on netixlan). Set to `0` to disable the escape hatch — only the per-cycle `MAX(updated)` cursor applies. |
...
|----------|------------|-------------|-----------|
| `PDBPLUS_INCLUDE_DELETED` | v1.16 | None — deleted rows are always persisted as tombstones (soft-delete) and exposed via `?since=N` / pk lookup per upstream `rest.py:694-727` status × since matrix. | Remove from your environment. The variable is now rejected at startup if set. |
