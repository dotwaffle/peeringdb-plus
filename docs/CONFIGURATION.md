...
| `PDBPLUS_FULL_SYNC_INTERVAL` | No | `24h` | Go duration | Interval after which a sync cycle forces a full bare-list refetch of every type, regardless of the per-table `MAX(updated)` cursor. The forced cycle also rewrites every row, because it turns off the upsert gate that skips a row whose `updated` did not advance. It is the only automatic repair for data that incremental sync cannot see: upstream count fields; `name`, `city` and `country` on `netfac`/`ixfac` and `name` on `carrierfac`, which upstream copies from the facility; live rows that a paged `?since=` fetch skipped because they share one `updated` value; values that upstream set before the mirror stored the field (for example `meta`); and FK columns that the sync set to `NULL` for a missing parent. See `docs/ARCHITECTURE.md` § Daily full reconcile. It also defends against pathological upstream cross-row inconsistency in any `?since=`-based design (a response with row R' (`updated=M`) present but earlier row R (`updated < M`) missing → R is permanently missed without periodic full refetch). The forced cycle is recorded in `sync_status.mode='full'`; `GetLastSuccessfulFullSyncTime` reads that column to decide whether the next cycle must escalate. Single sync_status query per cycle (NOT per-type). Forced-full cycles over a populated table also fetch `?since=<cursor>` per type on top of the bare snapshot so the window's tombstones are captured (bare lists carry only live rows: `status='ok'`, plus `not-operational` on netixlan). Set to `0` to disable the escape hatch — only the per-cycle `MAX(updated)` cursor applies, and the data listed above stays stale until an operator runs `POST /sync?mode=full`. |
