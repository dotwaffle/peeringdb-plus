...
// maxPageSize is the maximum number of items per page for cursor-based pagination (D-14).
const maxPageSize = 1000

// validatePageSize checks that first/last do not exceed maxPageSize.
func validatePageSize(first, last *int) error {
	if first != nil && *first > maxPageSize {
		return fmt.Errorf("first must not exceed %d, got %d", maxPageSize, *first)
	}
	if last != nil && *last > maxPageSize {
		return fmt.Errorf("last must not exceed %d, got %d", maxPageSize, *last)
	}
	return nil
}
...
// CampusSlice is the resolver for the campusSlice field.
func (r *queryResolver) CampusSlice(ctx context.Context, after *entgql.Cursor[int], first *int, before *entgql.Cursor[int], last *int, orderBy *ent.CampusOrder, where *ent.CampusWhereInput) (*ent.CampusConnection, error) {
