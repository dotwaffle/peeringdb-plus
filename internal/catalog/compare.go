...
func computeSharedIXPs(a, b []*ent.NetworkIxLan) []CompareIXP {
	...
	var shared []CompareIXP
	seen := make(map[int]bool)

	for _, nixl := range b {
		if nixlA, ok := ixMapA[nixl.IxID]; ok && !seen[nixl.IxID] {
			seen[nixl.IxID] = true
			shared = append(shared, CompareIXP{
				IXID:   nixl.IxID,
				IXName: nixlA.Name,
				Shared: true,
	...
	slices.SortFunc(shared, func(a, b CompareIXP) int { return cmp.Compare(a.IXName, b.IXName) })
	return shared
}

// computeSharedFacilities finds facilities where both networks are present by intersecting facility IDs.
func computeSharedFacilities(a, b []*ent.NetworkFacility) []CompareFacility {
	...
	for _, nf := range b {
		...
		if nfA, ok := facMapA[facID]; ok && !seen[facID] {
			...
			cf := CompareFacility{
				...
				Country: nfA.Country,
				Shared:  true,
				NetA:    &CompareFacPresence{LocalASN: nfA.LocalAsn},
				NetB:    &CompareFacPresence{LocalASN: nf.LocalAsn},
			}
	...
	slices.SortFunc(shared, func(a, b CompareFacility) int { return cmp.Compare(a.FacName, b.FacName) })
	return shared
}
...
func (s *CompareService) computeSharedCampuses(ctx context.Context, facNetsA, facNetsB []*ent.NetworkFacility) ([]CompareCampus, error) {
	...
	for campusID, facsA := range campusFacsA {
		...
		var sharedFacs []CompareCampusFacility
		for _, f := range facsB {
			if facSetA[f.facID] {
				sharedFacs = append(sharedFacs, CompareCampusFacility{
					FacID:   f.facID,
					FacName: f.facName,
				})
			}
		...
		shared = append(shared, CompareCampus{
			CampusID:         campusID,
	...
	slices.SortFunc(shared, func(a, b CompareCampus) int { return cmp.Compare(a.CampusName, b.CampusName) })
	return shared, nil
}
...
func computeAllIXPs(a, b []*ent.NetworkIxLan) []CompareIXP {
	type ixEntry struct {
		ixID   int
		ixName string
	...
	result := make([]CompareIXP, 0, len(entries))
	for _, e := range entries {
		result = append(result, CompareIXP{
			IXID:   e.ixID,
			IXName: e.ixName,
	...
	slices.SortFunc(result, func(a, b CompareIXP) int { return cmp.Compare(a.IXName, b.IXName) })
	return result
}

// computeAllFacilities produces a union of both networks' facility presences with shared flags.
func computeAllFacilities(a, b []*ent.NetworkFacility) []CompareFacility {
	...
	for _, nf := range a {
		...
		e := &facEntry{
			...
			city:    nf.City,
			country: nf.Country,
			netA:    &CompareFacPresence{LocalASN: nf.LocalAsn},
		}
		extractCoords(nf, e)
		entries[*nf.FacID] = e
	}
	...
	for _, nf := range b {
		...
		if e, ok := entries[facID]; ok {
			e.netB = &CompareFacPresence{LocalASN: nf.LocalAsn}
		} else {
			e := &facEntry{
				facID:   facID,
				facName: nf.Name,
	...
	result := make([]CompareFacility, 0, len(entries))
	for _, e := range entries {
		result = append(result, CompareFacility{
			FacID:     e.facID,
			FacName:   e.facName,
			City:      e.city,
			Country:   e.country,
	...
	slices.SortFunc(result, func(a, b CompareFacility) int { return cmp.Compare(a.FacName, b.FacName) })
	return result
}

// ixPresence converts an ent NetworkIxLan to a template CompareIXPresence.
func ixPresence(nixl *ent.NetworkIxLan) *CompareIXPresence {
	p := &CompareIXPresence{
		Speed:       nixl.Speed,
