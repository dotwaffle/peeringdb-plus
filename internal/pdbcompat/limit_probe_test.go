...
//
// One plausible hypothesis was that .Limit(0)
...
// The closures use the explicit `if opts.Limit > 0` form for
...
func TestEntLimitZeroProbe(t *testing.T) {
	...
	// the result that drives the `if opts.Limit > 0 { .Limit(...) }`
	...
	t.Run("Limit_0_returns_all_rows", func(t *testing.T) {
		...
		if len(rows) != 3 {
			t.Fatalf("ent .Limit(0) returned %d rows; want 3 (ent treats Limit(0) as unlimited via sqlgraph graph.go:1086 `if q.Limit != 0` gate). If this flips, re-examine the list closures.", len(rows))
		}
	})
