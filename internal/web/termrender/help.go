...
func (r *Renderer) RenderHelp(w io.Writer, freshness time.Time) error {
	...
	buf.WriteString("\n")
	buf.WriteString("  $ curl https://peeringdb-plus.fly.dev/ui/asn/<asn>\n")
	buf.WriteString("  $ curl https://peeringdb-plus.fly.dev/ui/ix/<id>\n")
	buf.WriteString("  $ curl https://peeringdb-plus.fly.dev/ui/fac/<id>\n")
	buf.WriteString("  $ curl https://peeringdb-plus.fly.dev/ui/org/<id>\n")
	buf.WriteString("  $ curl https://peeringdb-plus.fly.dev/ui/campus/<id>\n")
	buf.WriteString("  $ curl https://peeringdb-plus.fly.dev/ui/carrier/<id>\n")
	buf.WriteString("\n")

	// Search section.
	buf.WriteString(StyleHeading.Render("Search:"))
	buf.WriteString("\n")
	buf.WriteString("  $ curl https://peeringdb-plus.fly.dev/ui/?q=cloudflare\n")
	buf.WriteString("\n")

	// Compare section.
	buf.WriteString(StyleHeading.Render("Compare Networks:"))
	buf.WriteString("\n")
	buf.WriteString("  $ curl https://peeringdb-plus.fly.dev/ui/compare/13335/15169\n")
	buf.WriteString("\n")
	...
	buf.WriteString("\n")
	buf.WriteString("  $ curl https://peeringdb-plus.fly.dev/ui/asn/13335           # Cloudflare\n")
	buf.WriteString("  $ curl https://peeringdb-plus.fly.dev/ui/asn/15169           # Google\n")
	buf.WriteString("  $ curl https://peeringdb-plus.fly.dev/ui/ix/31               # DE-CIX Frankfurt\n")
	buf.WriteString("  $ curl \"https://peeringdb-plus.fly.dev/ui/asn/13335?T\"       # Plain text\n")
	buf.WriteString("  $ curl \"https://peeringdb-plus.fly.dev/ui/asn/13335?format=json\" | jq .\n")
	buf.WriteString("\n")
	...
	buf.WriteString("    $ eval \"$(curl -s https://peeringdb-plus.fly.dev/ui/completions/bash)\"\n")
	buf.WriteString("\n")
	buf.WriteString("  Quick setup (zsh):\n")
	buf.WriteString("    $ eval \"$(curl -s https://peeringdb-plus.fly.dev/ui/completions/zsh)\"\n")
	buf.WriteString("\n")
	buf.WriteString("  Another deployment (set PDB_HOST to its base URL):\n")
	buf.WriteString("    $ PDB_HOST=https://pdb.example.net\n")
	buf.WriteString("    $ eval \"$(curl -s \"$PDB_HOST/ui/completions/bash\")\"\n")
	buf.WriteString("\n")
	buf.WriteString("  Manual alias:\n")
	buf.WriteString("    $ pdb() { curl -s \"https://peeringdb-plus.fly.dev/ui/$@\"; }\n")
	buf.WriteString("\n")
