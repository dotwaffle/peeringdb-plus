...
- **D-02 — Path B mechanism: ent introspection via generated schema graph**. New `internal/pdbcompat/introspect.go` walks `client.Schema.Tables()` at app init (once, cached) to build `map[string][]Edge{name, target, field_names}`. Request-time filter parser consults this map when a `<fk>__<field>` param doesn't match a direct field. No hand-maintained registry.
...
- ent introspection: https://entgo.io/docs/sql-integration (Schema.Tables, Edge metadata)
