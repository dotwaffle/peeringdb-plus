...
status: completed
stopped_at: "Completed 70-01 — pdbcompat annotation types + AllowlistEntry value type. internal/pdbcompat/annotations.go (92 LOC) exports PrepareQueryAllowAnnotation + FilterExcludeFromTraversalAnnotation (both ent schema.Annotation; Names locked: PrepareQueryAllow / FilterExcludeFromTraversal) plus AllowlistEntry{Direct []string, Via map[string][]string} value type. Constructors WithPrepareQueryAllow(fields ...string) (copies caller slice per T-70-01-01) and WithFilterExcludeFromTraversal() (presence-as-signal, D-03). internal/pdbcompat/annotations_test.go (68 LOC, 4 tests / 7 subcases inc. table-driven TestWithPrepareQueryAllow, TestWithPrepareQueryAllow_NoAliasing, TestWithFilterExcludeFromTraversal, TestAllowlistEntry_Shape). go vet + go build + go test -race all PASS (6.492s on pdbcompat). golangci-lint run ./internal/pdbcompat/... = 0 issues. Grep acceptance: 13 type mentions (>=3), 2 Name() decls (==2). Commits: 268346b (feat) + 41f2ceb (test). Next: Plan 70-02 — cmd/pdb-compat-allowlist/ codegen tool."
last_updated: "2026-04-19T18:04:42.730Z"
...
Last session: 2026-04-19T18:04:36.056Z
Last activity: 2026-04-19
Stopped at: Completed 70-01 — pdbcompat annotation types + AllowlistEntry value type. internal/pdbcompat/annotations.go (92 LOC) exports PrepareQueryAllowAnnotation + FilterExcludeFromTraversalAnnotation (both ent schema.Annotation; Names locked: PrepareQueryAllow / FilterExcludeFromTraversal) plus AllowlistEntry{Direct []string, Via map[string][]string} value type. Constructors WithPrepareQueryAllow(fields ...string) (copies caller slice per T-70-01-01) and WithFilterExcludeFromTraversal() (presence-as-signal, D-03). internal/pdbcompat/annotations_test.go (68 LOC, 4 tests / 7 subcases inc. table-driven TestWithPrepareQueryAllow, TestWithPrepareQueryAllow_NoAliasing, TestWithFilterExcludeFromTraversal, TestAllowlistEntry_Shape). go vet + go build + go test -race all PASS (6.492s on pdbcompat). golangci-lint run ./internal/pdbcompat/... = 0 issues. Grep acceptance: 13 type mentions (>=3), 2 Name() decls (==2). Commits: 268346b (feat) + 41f2ceb (test). Next: Plan 70-02 — cmd/pdb-compat-allowlist/ codegen tool.
