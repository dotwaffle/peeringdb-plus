...
Last activity: 2026-04-19
Stopped at: Completed 69-04 — pdbcompat filter layer for UNICODE-01/02 + IN-01/02. ParseFilters now takes TypeConfig (not a bare Fields map) so it can consult tc.FoldedFields; returns (preds, emptyResult, err) so empty `?field__in=` short-circuits with zero SQL. 16 folded fields across 6 types route substring/prefix/iexact via <field>_fold LIKE ? with unifold.Fold(value) on RHS. __in uses sql.ExprP("... IN (SELECT value FROM json_each(?))", jsonStr) — single JSON-array bind regardless of list size. Inner-loop err renamed to parseErr in buildIn int path (W1 fix). Phase 68 status × since matrix preserved in all 13 list closures (grep verified). Rule 1 bug auto-fixed during GREEN: setupTestHandler seeded Network rows without name_fold, failing TestQueryFilterContains under the new routing; fix added .SetNameFold(unifold.Fold(name)) to the 3 seeds + unifold import, rolled into the GREEN commit (inseparable — production sync populates _fold, direct ent.Create bypasses it). New test file phase69_filter_test.go with 8 functions/13 subtests covering Zürich diacritic matching, 1500-element __in, empty __in, EXPLAIN QUERY PLAN json_each probe, D-04 scope guards. Commits: 9839273 (test RED) + 9aa661d (feat GREEN). Acceptance: 13 opts.EmptyResult guards, 13 applyStatusMatrix intact, 1 json_each SQL fragment, 7 unifold.Fold references in filter.go. Full suite + vet + golangci-lint all green.
