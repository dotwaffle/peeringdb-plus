...
last_activity: 2026-05-03 -- Completed quick task 260503-fw7: trim otelhttp + otelconnect cardinality via SDK Views in internal/otel/provider.go. Drops the entire rpc.server.* family (unused by every dashboard and alert), coarsens http.server.request.duration buckets to {0.01, 0.05, 0.25, 1, 5} (mirroring the pre-existing rpc duration View it replaced), and strips http.method / scheme / server.address attributes from the surviving HTTP duration histogram + active_requests gauge via AllowKeysFilter. Estimated reduction: per-machine ~6,800 → ~1,200 active series (~80% cut), fleet-wide ~50k → ~10k. New views_test.go uses ManualReader for hermetic assertions. Single atomic commit, all gates green (test -race / build / vet / golangci-lint clean). No dashboard or alert edits required — verified existing PromQL labels survive.
progress:
  total_phases: 0
  completed_phases: 0
  total_plans: 0
