...
status: completed
stopped_at: "Completed 70-02 — cmd/pdb-compat-allowlist codegen tool + wire into go generate. cmd/pdb-compat-allowlist/main.go (299 LOC) reads ent schema graph via entc.LoadGraph and emits internal/pdbcompat/allowlist_gen.go with Allowlists + FilterExcludes maps; hop-splitting via strings.Split on __ (2 segs → Direct, 3 segs → Via[firstHop], 4+ → warn-drop per D-04). Deterministic sort-before-render for byte-stable output — two-run SHA256 PASS (6b0857fd...). Bootstrap emission is empty (schemas don't carry annotations yet — Plan 70-03 populates). decodeFields handles both map[string]any (JSON-roundtrip) and fieldser interface fallback. pdbTypeFor locks the 13-entity Go-name → PeeringDB-type mapping. main_test.go (99 LOC, 3 tests / 8 subcases). ent/generate.go adds //go:generate step between ent codegen and buf generate. .gitignore extended with /pdb-compat-allowlist + /pdbcompat-check. go build + go vet + go test -race ./cmd/pdb-compat-allowlist/ + ./internal/pdbcompat/ all PASS. Commit: dd8ffcc. Next: Plan 70-03 — attach WithPrepareQueryAllow annotations to all 13 ent schemas."
...
Phase: 70 (cross-entity-traversal) — IN PROGRESS (2/8 plans shipped: 70-01 annotation types, 70-02 codegen tool)
Plan: Last completed 70-02 (cmd/pdb-compat-allowlist codegen tool + wire into `go generate`). Tool reads ent schema graph via entc.LoadGraph and emits internal/pdbcompat/allowlist_gen.go with Allowlists + FilterExcludes maps; byte-stable two-run idempotency confirmed (SHA 6b0857fd...). Bootstrap emission is empty — Plan 70-03 attaches WithPrepareQueryAllow(...) to all 13 schemas and re-running the tool populates it. ent/generate.go now runs the tool between ent codegen and buf generate.
Status: Plan 70-02 shipped cleanly. Single atomic commit dd8ffcc (feat). go build + go vet + go test -race ./cmd/pdb-compat-allowlist/ + ./internal/pdbcompat/ PASS. Verification: two-run SHA256 equal, `grep -c 'pdb-compat-allowlist' ent/generate.go` == 1, `head -2 internal/pdbcompat/allowlist_gen.go | grep -c 'Code generated'` == 1. Deviation: added .gitignore entries for /pdb-compat-allowlist + /pdbcompat-check (Rule 3 — stray `go build` binary in repo root).
Next action: `/gsd-execute-phase 70 --plan 03` — annotate all 13 ent schemas with WithPrepareQueryAllow(...) fields translated from upstream `peeringdb_server/serializers.py:1823, 2244, 2361, 2423, 2573, 2732, 2947, 2995, 3315, 3451, 3622, 3925, 4041` get_relation_filters(...) lists.
...
Last activity: 2026-04-19
Stopped at: Completed 70-02 — cmd/pdb-compat-allowlist codegen tool + wire into `go generate`. cmd/pdb-compat-allowlist/main.go (299 LOC) reads ent schema graph via entc.LoadGraph and emits internal/pdbcompat/allowlist_gen.go with Allowlists + FilterExcludes maps; hop-splitting on "__" (2 segs → Direct, 3 segs → Via[firstHop], 4+ → warn-drop per D-04); deterministic sort-before-render — two-run SHA256 PASS (6b0857fd...). Bootstrap emission is empty (schemas don't carry annotations yet — Plan 70-03 populates). decodeFields handles both map[string]any (JSON-roundtrip ent default) and fieldser interface fallback. pdbTypeFor locks 13-entity Go-name → PeeringDB-type mapping. main_test.go (99 LOC, 3 tests / 8 subcases). ent/generate.go adds middle `//go:generate` step between ent codegen and buf generate. .gitignore extended with /pdb-compat-allowlist + /pdbcompat-check (Rule 3 auto-fix — stray `go build` binary). go build + go vet + go test -race ./cmd/pdb-compat-allowlist/ + ./internal/pdbcompat/ PASS. Single atomic commit: dd8ffcc. Next: Plan 70-03 — attach WithPrepareQueryAllow to all 13 ent schemas from upstream serializers.py:{1823, 2244, 2361, 2423, 2573, 2732, 2947, 2995, 3315, 3451, 3622, 3925, 4041} get_relation_filters lists.
