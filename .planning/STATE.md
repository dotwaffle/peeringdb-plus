...
last_activity: 2026-05-03 -- Completed quick task 260503-huo: invert per-route OTel head sampler default in internal/otel/provider.go. DefaultRatio drops from in.SampleRate (1.0) to hardcoded 0.01; in.SampleRate is redirected onto the four known app surfaces (/api/, /rest/v1/, /peeringdb.v1., /graphql) so the operator escape hatch still works for legitimate traffic; new explicit deny-prefixes /. (0.001) and /wp- (0.001) catch dotfile and WordPress probes at the boundary. Triggered by a vulnerability scanner from 45.148.10.238 (UA SecurityScanner/1.0) that consumed ~2 GB of the 50 GB/month free-tier trace budget on day 2 of the month, peaking at 384 KB/s vs the free-tier 500 KB/s cap and tripping live_traces_exceeded discards. Single atomic commit, all gates green (test -race, build, vet, golangci-lint clean). Estimated reduction on next scanner sweep: ~100x (~2 GB spike → ~20 MB).
progress:
  total_phases: 0
  completed_phases: 0
  total_plans: 0
