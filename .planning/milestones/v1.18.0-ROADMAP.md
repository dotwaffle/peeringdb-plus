...
- ✅ **v1.18.0 — Cleanup & Observability Polish** — shipped 2026-04-27 (Phases 73-78, 15 requirements)

## Phases

**v1.18.0 — Cleanup & Observability Polish (current)**

- [x] **Phase 73: Code Defect Fixes** — Resolve the two known production bugs (campus inflection 500 in cross-entity traversal; `poc.role` NotEmpty validator extending the codegen-layer guard from 260426-pms)
- [x] **Phase 74: Test & CI Debt** — Three deferred test failures (`TestGenerateIndexes`, `TestDashboard_RegionVariableUsed`) + 5 lint findings in `internal/visbaseline` flipped clean
- [x] **Phase 75: Code-side Observability Fixes** — Cold-start gauge populate (OBS-01), zero-rate counter pre-warm (OBS-02), `http.route` middleware investigation (OBS-04)
- [x] **Phase 76: Dashboard Hardening** — `service_name` filter sweep across `go_*` panel queries (OBS-03); confirm `pdbplus_response_heap_delta_bytes_bucket` flow on v1.17.0+ binary (OBS-05)
- [x] **Phase 77: Telemetry Audit & Cleanup** — Loki log-level audit (OBS-06); trace sampling/batching review against PERF-08 baseline (OBS-07)
- [x] **Phase 78: UAT Closeout** — v1.13 CSP enforcement verification (UAT-01), v1.13 security headers + body cap + slowloris (UAT-02), v1.5 Phase 20 archive (UAT-03)
