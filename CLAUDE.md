...
- Metrics: Fly.io scrapes LiteFS `:20202/metrics` on every machine (`fly.toml` `[[metrics]]`, 15s) into the Fly.io managed Prometheus (Grafana data source `fly.io`), as native `litefs_*` with labels `app`/`instance`/`region`/`host` only: split primary/replica with `litefs_is_primary` joined `on (instance)`.
