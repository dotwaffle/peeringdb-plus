...
### Code Generation
- `go generate ./ent` runs entc.go (ent codegen + entproto). Do NOT run `go generate ./schema` after entproto annotations are added — the schema generator strips entproto annotations.
- `templ generate ./internal/web/templates/` regenerates templ Go files. Always commit `*_templ.go` alongside `.templ` changes.
- `buf generate` regenerates proto Go types. Run after editing `.proto` files. Generated output: `gen/peeringdb/v1/`.
